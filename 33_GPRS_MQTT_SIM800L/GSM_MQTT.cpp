...
//#define MEGA2560
#ifdef ESP12E
  SoftwareSerial UART_NAME(D3,D2);
#endif
#ifdef MEGA2560
 #define UART_NAME              Serial1
 //SoftwareSerial UART_NAME(3,2);
#endif

/* User can change this extern variable, depend on variable from main program */

extern GSM_MQTT MQTT; //re-decleration as extern variable from main program.

/* Define for MQTT Cloud */
char *MQTT_HOST="broker.drinkwellatm.com";
...
void GSM_MQTT:: setHostPort(char *host, uint16_t port){
  MQTT_HOST = host;
  MQTT_PORT = port;
  this->begin();
}
...
void serialEvent_private(void)
{
//#if (UART_NAME == Serial)
    serialEvent();
//#else if (UART_NAME == Serial1)
//    serialEvent1();
//#endif
}
...
void serial_function(void)
{
    ...
    while (UART_NAME.available())
    {
        char inChar = (char)UART_NAME.read();
        if (MQTT.TCP_Flag == false)
        {
            ...
            if (inChar == '\n')
            {
                ...
                stringComplete = true;
#ifdef USING_DEBUG
                UART_DEBUG.print(MQTT.inputString);
#endif
                ...
                else if (strstr(MQTT.inputString, "CONNECT") != NULL)
                {
                    ...
                    MQTT.TCP_Flag = true;
#ifdef USING_DEBUG
                    UART_DEBUG.println("MQTT.TCP_Flag = True");
#endif
                    MQTT.AutoConnect();
                    MQTT.pingFlag = true;
        ...
        else
        {
            ...
            if ((ReceivedMessageType >= CONNECT) && (ReceivedMessageType <= DISCONNECT))
            {
                ...
                {
                    if (UART_NAME.available())
                    {
                        inChar = (char)UART_NAME.read();
#ifdef USING_DEBUG
                        UART_DEBUG.println(inChar, DEC);
#endif
                        if ((((Cchar & 0xFF) == 'C') && ((inChar & 0xFF) == 'L') && (MQTT.length == 0)) || (((Cchar & 0xFF) == '+') && ((inChar & 0xFF) == 'P') && (MQTT.length == 0)))
                        {
                            ...
                            MQTT.pingFlag = false;
#ifdef USING_DEBUG
                            UART_DEBUG.println("Disconnecting");
#endif
                        }
                        else
                        {
...
#ifdef USING_DEBUG
                UART_DEBUG.println(MQTT.length);
#endif
                if (MQTT.TCP_Flag == true)
                {
                    ...
                    MQTT.index = 0L;
                    uint32_t a = 0;
                    while ((MQTT.length-- > 0) && (UART_NAME.available()))
                    {
                        MQTT.inputString[uint32_t(MQTT.index++)] = (char)UART_NAME.read();
...
#ifdef USING_DEBUG
                        UART_DEBUG.print("Message ID :");
                        UART_DEBUG.println(MQTT.inputString[0] * 256 + MQTT.inputString[1]) ;
#endif
                    }
            ...
            {
#ifdef USING_DEBUG
                UART_DEBUG.print("Received :Unknown Message Type :");
                UART_DEBUG.println(inChar);
#endif
            }
        }
    }
...
{
    UART_NAME.begin(UART_BAUD);
#ifdef USING_DEBUG
    UART_DEBUG.begin(UART_DEBUG_BAUD);
#endif
    UART_NAME.write("AT\r\n");
    #ifdef UART_DEBUG
      UART_DEBUG.println("Debug begin");
    #endif
    delay(1000);
    _tcpInit();
}
...
char GSM_MQTT::_sendAT(char *command, unsigned long waitms)
{
    ...
    #ifdef UART_DEBUG
    UART_DEBUG.println(GSM_Response);
    #endif
    return GSM_Response;
}
...
char GSM_MQTT::sendATreply(char *command, char *replystr, unsigned long waitms)
{
    ...
    #ifdef UART_DEBUG
      UART_DEBUG.println(GSM_ReplyFlag);
    #endif
...
void GSM_MQTT::_tcpInit(void)
{
    ...
    case 3:
    {
        ...
        case 5:
        {
            UART_NAME.print("AT+CIPSTART=\"TCP\",\"");
            UART_NAME.print(MQTT_HOST);
            UART_NAME.print("\",\"");
            UART_NAME.print(MQTT_PORT);
            if (_sendAT("\"\r\n", 5000) == 1)
            {
                unsigned long PrevMillis = millis();
                unsigned long currentMillis = millis();
                ...
                while ( (GSM_Response != 4) && ((currentMillis - PrevMillis) < 30000) )
                {
                    serialEvent_private();
                    currentMillis = millis();
                    if (currentMillis < PrevMillis)
...
void GSM_MQTT::_ping(void)
{
    ...
    if (pingFlag == true)
    {
        ...
        {
            _PingPrevMillis = currentMillis;
            UART_NAME.print(char(PINGREQ * 16));
            _sendLength(0);
        }
    }
}
...
void GSM_MQTT::_sendUTFString(char *string)
{
    ...
    UART_NAME.print(char(localLength / 256));
    UART_NAME.print(char(localLength % 256));
    UART_NAME.print(string);
}
...
void GSM_MQTT::_sendLength(int len)
{
    ...
    while (length_flag == false)
    {
        if ((len / 128) > 0)
        {
            UART_NAME.print(char(len % 128 + 128));
            len /= 128;
        }
        else
        {
            length_flag = true;
            UART_NAME.print(char(len));
        }
    }
}
...
{
    ConnectionAcknowledgement = NO_ACKNOWLEDGEMENT ;
    UART_NAME.print(char(CONNECT * 16 ));
    ...
    UART_NAME.print(char(_ProtocolVersion));
    UART_NAME.print(char(UserNameFlag * User_Name_Flag_Mask + PasswordFlag * Password_Flag_Mask + WillRetain * Will_Retain_Mask + WillQoS * Will_QoS_Scale + WillFlag * Will_Flag_Mask + CleanSession * Clean_Session_Mask));
    UART_NAME.print(char(_KeepAliveTimeOut / 256));
    UART_NAME.print(char(_KeepAliveTimeOut % 256));
...
{
    UART_NAME.print(char(PUBLISH * 16 + DUP * DUP_Mask + Qos * QoS_Scale + RETAIN));
    ...
    if (Qos > 0)
    {
        UART_NAME.print(char(MessageID / 256));
        UART_NAME.print(char(MessageID % 256));
    }
    UART_NAME.print(Message);
}
...
{
    UART_NAME.print(char(PUBACK * 16));
    _sendLength(2);
    UART_NAME.print(char(MessageID / 256));
    UART_NAME.print(char(MessageID % 256));
}
...
{
    UART_NAME.print(char(PUBREC * 16));
    _sendLength(2);
    UART_NAME.print(char(MessageID / 256));
    UART_NAME.print(char(MessageID % 256));
}
...
{
    UART_NAME.print(char(PUBREL * 16 + DUP * DUP_Mask + 1 * QoS_Scale));
    _sendLength(2);
    UART_NAME.print(char(MessageID / 256));
    UART_NAME.print(char(MessageID % 256));
}
...
{
    UART_NAME.print(char(PUBCOMP * 16));
    _sendLength(2);
    UART_NAME.print(char(MessageID / 256));
    UART_NAME.print(char(MessageID % 256));
}
...
{
    UART_NAME.print(char(SUBSCRIBE * 16 + DUP * DUP_Mask + 1 * QoS_Scale));
    int localLength = 2 + (2 + strlen(SubTopic)) + 1;
    _sendLength(localLength);
    UART_NAME.print(char(MessageID / 256));
    UART_NAME.print(char(MessageID % 256));
    _sendUTFString(SubTopic);
    UART_NAME.print(SubQoS);
...
{
    UART_NAME.print(char(UNSUBSCRIBE * 16 + DUP * DUP_Mask + 1 * QoS_Scale));
...
{
    UART_NAME.print(char(DISCONNECT * 16));
    _sendLength(0);
    pingFlag = false;
}
...
void GSM_MQTT::OnConnect(void)
{
     ...
     */
    #ifdef UART_DEBUG
      UART_DEBUG.println("Connected to broker");
    #endif
    subscribe(0, _generateMessageID(), "inTopic", 0);
